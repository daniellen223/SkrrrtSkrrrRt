...
M = 24                          # Number of hidden nodes - 12 dimensional data
...
for i in range(len(learning_rate)):
    ...
    tools.plot_results(100*first_testing_loss, filename=f"Fig1Learnrate{learning_rate[i]}", ylabel="% error", title="Before training")
    tools.plot_results(100*testing_loss, filename=f"Fig2Learnrate{learning_rate[i]}", ylabel="% error", title="After training")
    print(Fore.GREEN + "Complete" + Style.RESET_ALL)
    ...
    print("\nmain.py ran successfully in {:0.1f} minutes\n".format(runtime/60))
